...
app.post("/register",(req,res)=>{
    ...
    let id =uuidv4();
    let q = `INSERT INTO UserDetails(id, userName, email, password) VALUES ('${id}','${userName}','${email}','${password}')`;
...
app.post("/adminRegistration",(req,res)=>{
    ...
    return res.status(400).json({
      success: false,
      message: "Please provide adminName, email, and password"
    });
  }

    const checkQuery = "SELECT * FROM admin WHERE email = ? OR adminName = ?";
    connection.query(checkQuery, [email, adminName], async (err, results) => {
    if (err) throw err ;
    ...
    let q = `INSERT INTO admin(adminName,email,password) VALUES(?, ?, ?)`;

    connection.query(q,[adminName,email,password],(err,result)=>{
        if(err) throw err; 
...
app.patch("/admin/user/:id",(req,res)=>{
    ...
    connection.query(q,[id],(err,result)=>{
        ...
        let q = `UPDATE UserDetails SET userName = ?, email = ? WHERE  id =?`;
